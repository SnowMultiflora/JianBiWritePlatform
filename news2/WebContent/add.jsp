...
<%
	...
	Map resultMap = actionBean.queryNewsByPage(p, cid, ntitle,aid);
...
<body>
	...
	<div class="main-frame">
		...
		<div class="right">
			<div class="admin">
				<h1 class="title">已投稿文章</h1>

				<table class="tab" cellspacing="0">
					<tr>
						<td colspan="5">
						  <form action="add.jsp" method="post" id="frm">
		    <label>文章类别：</label>
		    <select name="cid">
		    ...
		    <input type="submit" value="查询">
					...
					<tr>
						<td><input type="radio" name="rad"
							value="<%=map.get("nid")%>" autocomplete="off" /></td>
						<td><%=++i%></td>
						<td><%=map.get("ntitle")%></td>
						<td><%=map.get("cname")%></td>
						<td><%=map.get("ndate")%></td>
					...
					%>
					<tr>
						<td colspan="5" class="page"><span>总共<%=rows%>篇文章
						</span><span>当前是第<%=p + 1%>页
						</span> <%
 	if (p > 0) {
 ...
 %> <a href="javascript:void(0)"
							onclick="nextPage(<%=p + 1%>)">下一页</a> <a href="javascript:void(0)"
							onclick="nextPage(<%=totalPages - 1%>)">尾页</a> <%
 	}
 %> <span>共<%=totalPages%>页
						</span> <%
 	if (totalPages > 1) {
 %> <span>跳转到第<select
								onchange="nextPage(this.value)">
									<%
										for (int j = 0; j < totalPages; j++) {
									%><option value="<%=j%>"
										<%=j == p ? "selected='selected'" : ""%>><%=j + 1%></option>
									<%
										}
									%>
							</select>页
 ...
 %></td>
	...
	<div class="win-box" id="win-box">
		<h2 class="win-title">
			修改文章 类别<span onclick="closeClassWindow(0);">&times;</span>
		</h2>
			...
			if (confirm("您确认要删除当前文章吗？")) {
				...
				$.get("/news2/DeleteNewsServlet", {
					...
					if ("power" == data) {
						...
						alert("文章删除成功");
						window.location.href = "DeleteNewsServlet?nid=" + obj.value;
					}
