...
public class ActionBean2 {
	public String queryuserAll(){
		...
		int i=1;
		for(User user:list){
			tableHTML+="<tr><td><input type=\"radio\" name=\"rad\" autocomplete=\"off\" value=\""+user.getAid()+"\"/></td><td class=\"text-center\">"+(i++)+"</td><td class=\"text-center\">"+user.getAid()+"</td><td class=\"text-center\">"+user.getAname()+"</td><td class=\"text-center\">"+user.getPhone()+"</td><td class=\"text-center\">"+user.getQq()+"</td></tr>";
		}
		sqlHelper.destroy();
		return tableHTML;
	}
