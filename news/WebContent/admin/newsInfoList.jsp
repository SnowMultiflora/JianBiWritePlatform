...
<%
	...
	/* List <NewsPojo>list=actionBean.queryNews();
    List <Map>list=actionBean.queryNewsMap(); */
   ...
   Map resultMap=actionBean.queryNewsByPage(p,cid,ntitle);
...
<div class="right">
	<div class="admin">
		<h1 class="title">文章信息管理<a href="admin/welcome.jsp" class="back-btn">返回主页</a></h1>
	
		<table class="tab" cellspacing="0">
		<tr>
		  <td colspan="5">
		  <form action="admin/newsInfoList.jsp" method="post" id="frm">
		    <label>文章类别：</label>
		    <select name="cid">
		 ...
		 <tr><td style="10px">选择</td><td>序号</td><td>文章标题</td><td>文章类别</td><td>发布时间</td>
		   ...
		   for(Map map:list){
			   %>
			   <tr><td><input type="radio" name="rad" value="<%=map.get("nid")%>" autocomplete="off"/></td><td><%=++i %></td><td><%=map.get("ntitle") %></td><td><%=map.get("cname") %></td><td><%=map.get("ndate") %></td></tr>
			   <%
		   }
		 %>   
		  <tr>
		 ...
		 <a class="btn" href="admin/addNews.jsp">增加文章</a>
...
<div class="win-box" id="win-box">
 ...
 <iframe src="" id="update-win"></iframe>
	...
	function updateNews(){
		...
		if(obj==null)return;
		//发起ajax请求进行用户的修改
				$.get("/news/admin/UpdateNewsServlet", {
				...
				if("power"==data){
				  alert("权限不足");
					...
					location="admin/updateNews.jsp?nid="+obj.value;
					}
	...
	function deleteNews(){
		...
		//发起ajax请求进行用户的删除
				$.get("/news/admin/DeleteNewsServlet", {
				...
				if("power"==data){
				  alert("权限不足");
						...
						alert("文章删除成功");
						window.location.href = "admin/DeleteNewsServlet?nid="+obj.value;
					}
